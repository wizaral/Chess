...
void ClassicFactory::init_game(Logic::Board &board, int &player_index, std::array<std::unique_ptr<Logic::Player>, 2> &players, Publisher *publisher) {
    ...
    if (p0c == Logic::FigureColor::Light) {
        ...
        board.add_figure({Logic::FigureType::King, Logic::FigureColor::Light, std::make_unique<Logic::KingStrategy>()}, {Logic::player0_figures_row, 4});
        board.add_figure({Logic::FigureType::Queen, Logic::FigureColor::Dark, std::make_unique<Logic::QueenStrategy>()}, {Logic::player1_figures_row, 3});
        board.add_figure({Logic::FigureType::King, Logic::FigureColor::Dark, std::make_unique<Logic::KingStrategy>()}, {Logic::player1_figures_row, 4});
    } else {
        board.add_figure({Logic::FigureType::Queen, Logic::FigureColor::Dark, std::make_unique<Logic::QueenStrategy>()}, {Logic::player0_figures_row, 4});
        board.add_figure({Logic::FigureType::King, Logic::FigureColor::Dark, std::make_unique<Logic::KingStrategy>()}, {Logic::player0_figures_row, 3});
        board.add_figure({Logic::FigureType::Queen, Logic::FigureColor::Light, std::make_unique<Logic::QueenStrategy>()}, {Logic::player1_figures_row, 4});
        board.add_figure({Logic::FigureType::King, Logic::FigureColor::Light, std::make_unique<Logic::KingStrategy>()}, {Logic::player1_figures_row, 3});
    }
